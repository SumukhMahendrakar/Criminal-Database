...
<head>
    ...
    if(isset($_POST['status'])){
        if($_SERVER["REQUEST_METHOD"]=="POST")
        {
            ...
            $qu1=mysqli_query($conn,"insert into update_case(c_id,case_update, criminal_id) values('$cid','$upd','$criminalid')");
            $q2=mysqli_query($conn,"update complaint set pol_status='ChargeSheet Filed' where c_id='$cid'");
            $quer3=mysqli_query($conn, "select type_crime, description from complaint where c_id='$cid'");
            $crimerows=mysqli_fetch_assoc($quer3);
            $sql4="insert into crimes(criminal_id, crime_type, crime_description) values ('$criminalid', '{$crimerows['type_crime']}', '{$crimerows['description']}')";
            $qu3=mysqli_query($conn, $sql4);
...
?>
  ...
  <div style="width: 100%; height: 250px;"> 
    ...
    <div style="width: 50%;margin-left: 28%;height: 250px; background-color: #dcdcdc"> 
     ...
     <form method="post">
      ...
      <select class="form-control" style="margin-left:30%; margin-top: 17px; width:250px;" name="criminalid">
            <?php
                        $cid = mysqli_query($conn, "SELECT `cri_id` FROM criminals");
                        while($row1 = mysqli_fetch_assoc($cid))
                        {
                            ?>
                                  <option> <?php echo $row1['cri_id']; ?> </option>
                            <?php
                        }
                        ?>
